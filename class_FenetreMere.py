...
import class_Regle
import class_Renommage
import class_Action
class FenetreMere(Frame):
    def __init__(self, master):
        ...
        self.barremenu.add_cascade(label = "?", underline = 0, menu = self.aide, command = self.apropos)
        ...
        label = Label(top, text="Renommer en lots")
        label.pack()      
        ...
        #BOT TOP
        bot_top = LabelFrame(bot, padx=0, pady=10, relief = FLAT)
        bot_top.pack(fill="both", expand="no", side=TOP)
        btnchoixrep = tk.Button(bot_top, text='Choisir votre repertoire', command = self.choixrep)
        btnchoixrep.pack()
        #BOT left
        ...
        nom_autre = StringVar()
        nom_autre.set("Nom personnalise")
        self.ent_autre = Entry(bot_right_left_right, textvariable=nom_autre, width=20)
        ...
        liste2= [("Image (png)", 1),("Video (avi)", 2),("Texte (txt)", 3),("Tout", 4)]
        for text, mode in liste2:
            b2 = Radiobutton(bot_right_right_right, text=text, variable=self.v2, value=mode)
            b2.pack(anchor=W)
        
        self.rep = False
        Button(bot_right_right_right, text ='Renommer', command=self.renommer).pack(side = BOTTOM)
    ...
    def renommer (self):
        ...
        if self.rep:
            ren = class_Renommage.Renommage(self.rep, ma_regle)
            ren.rename()
        else:
            messagebox.showerror("Erreur", "Vous n'avez pas selectioner le repertoire")
